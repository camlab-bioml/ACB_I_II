...
subtitle: "Advanced Computational Biology I"
...
**Non reproducible**: blind panic because you can't reproduce the figure<sup>1</sup>

**Reproducible**: easily rerun your computational workflow (specifying higher resolution), head to bar to celebrate paper acceptance

.footnote[
[1] Definitely not based on a true story
]
...
E.g. in **Snakemake**
```python
cohorts = ['Smith','Lin','Schapiro'] # Datasets
num_clusters = range(2,30) # Number of clusters
cluster_method = ['k_means','GMM'] # Clustering method

output_files = expand("output/cluster_results_{c}_{n}_{m}.tsv",
  c=cohorts,
  n=num_clusters,
  m=cluster_method)
```
